...
class ExpandOp(Op):
    def __init__(self, name: str, split_func: Callable[[Dict], (Dict, Dict)] = None):
        """Constructor of ExpandOp class for uniflow.

        Args:
            name (str): Name of the op.
            split_func (Callable[[Dict], (Dict, Dict)]): Function to split the value_dict into two.
        """
        super().__init__(name)
        self.split_func = split_func or self._default_split

    def _default_split(self, value_dict: Dict) -> (Dict, Dict):
    ...
    def __call__(self, root: Node) -> (Node, Node):
        ...
        expand_1 = Node(value_dict=value_dict_1)
        expand_2 = Node(value_dict=value_dict_2)
